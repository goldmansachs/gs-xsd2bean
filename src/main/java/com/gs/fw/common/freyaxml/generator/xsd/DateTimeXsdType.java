...
{
    public DateTimeXsdType(String prefix)
    {
        this.prefix = prefix;
    }
    ...
    {
        return "new java.text.SimpleDateFormat(\"yyyy-MM-dd'T'HH:mm:ss.SSSZ\").parse(\""+defaultValue.substring(0, Math.min(defaultValue.length(), 10))+"\")";
    }
    ...
    public String getMarshallerWriteElement(String choiceVariableName, String name, String singularVariableName, String boxedJavaType)
    {
        ...
        return "marshaller.writeDateTime(\""+name+"\", "+ boxedJava +" "+ variable+")";
    }

    @Override
    public String getMarshallerWriteAttribute(String name, String variableName)
    {
        return "marshaller.writeDateTimeAttribute(\""+name+"\", "+variableName+");";
    }
}
