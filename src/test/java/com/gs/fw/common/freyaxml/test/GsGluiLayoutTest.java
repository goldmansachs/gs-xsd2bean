...
public class GsGluiLayoutTest
{
    @Test
    public void testAllFiles() throws IOException
    {
        File dir = new File("./src/test/testdata/gs-glui");
        File[] files = dir.listFiles();
        Assert.assertTrue(files.length > 0);
        long start = System.currentTimeMillis();
        for(File f: files)
        {
            if (f.getName().endsWith(".gml2"))
            {
                new GsGluiLayoutUnmarshaller().parse(f.getCanonicalPath());
            }
        }
    ...
    public void testRoundTripFiles() throws Exception
    {
        ...
        for(File f: files)
        {
            if (f.getName().endsWith(".gml2"))
            {
                Object parsed = new GsGluiLayoutUnmarshaller().parse(f.getCanonicalPath());
    ...
    private Object parse(StringBuilder marshalled, String originalFilename) throws Exception
    {
        try
        {
            return new GsGluiLayoutUnmarshaller().parse(new ByteArrayInputStream(marshalled.toString().getBytes()), "remarshalled "+originalFilename);
        }
        catch (Exception e)
        {
    ...
    {
        GsGluiLayoutMarshaller marshaller = new GsGluiLayoutMarshaller();
        marshaller.setIndent(true);
        StringBuilder builder = new StringBuilder();
        Method method = GsGluiLayoutMarshaller.class.getMethod("marshall", Appendable.class, parsed.getClass());
