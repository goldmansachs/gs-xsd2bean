...
import com.gs.fw.common.freyaxml.test.fwcommon.desktop.FWCommonDesktopMarshaller;
import com.gs.fw.common.freyaxml.test.fwcommon.desktop.FWCommonDesktopUnmarshaller;
...
public class FwDesktopTest
{
    @Test
    public void testAllFiles() throws IOException
    {
        File dir = new File("./src/test/testdata/fwcommon/desktop");
    ...
    public void testRoundTripFiles() throws Exception
    {
        ...
        for(File f: files)
        {
            if (f.getName().endsWith(".xml"))
            {
                Object parsed = new FWCommonDesktopUnmarshaller().parse(f.getCanonicalPath());
    ...
    {
        return new FWCommonDesktopUnmarshaller().parse(new ByteArrayInputStream(marshalled.toString().getBytes()), "remarshalled "+originalFilename);
    }

    public StringBuilder marshall(Object parsed) throws Exception
    {
        FWCommonDesktopMarshaller marshaller = new FWCommonDesktopMarshaller();
        marshaller.setIndent(true);
        StringBuilder builder = new StringBuilder();
        Method method = FWCommonDesktopMarshaller.class.getMethod("marshall", Appendable.class, parsed.getClass());
