...
public class Libboot
{
    ...
    private static final int TODO_REWRITE = 10;
    ...
    private static class Download
    {
                        ...
                        try
                        {
                            ...
                            while(line != null)
                            {
                                ...
                                if (!line.startsWith("#") && !line.isEmpty())
                                {
                                    ...
                                    if (resultSpec != null)
                                    {
                                        Spec targetSpec = specsByKeyInOneDirectory.get(resultSpec.getKey());
                                        if (targetSpec == null)
                                        {
        ...
        private Map<String, Map<String, Spec>> organizeSpecs()
        {
            ...
            for(int i=0;i<specs.size();i++)
            {
                ...
                if (specsForDir.put(someSpec.getKey(), someSpec) != null)
                {
    ...
    {
        String result = e.getClass().getSimpleName() + ": " + e.getMessage();
    ...
    private static class Spec
    {
        ...
        public String getFilename()
        {
            if (filename == null)
            {
                filename = this.artifactId+"-"+this.version+"."+this.extension;
            }
            return filename;
        }
            ...
            if (delta == ALL_GOOD)
            {
                ...
                else
                {
                    ...
                    if (this.checksum == null)
                    {
                        ...
                        download.specToDo = TODO_REWRITE;
                    }
                }
            }
    ...
    private static IOException enhanceExceptionIfHttpConnection(String fileUrl, IOException originalException, URLConnection conn)
    {
        ...
        return new IOException(exceptionDetails.toString(), originalException);
    }
